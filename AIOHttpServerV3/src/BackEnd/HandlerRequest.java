...
class HandleRequest extends Thread {
    ...
    public HandleRequest(Socket sock, Hashtable knownRequests) {
        ...
        this.knownRequests.put("Counter", this.knownRequests.get("Counter") + new String("a"));
        //开始线程，为来自客户端的请求服务
        start();
    }
    // 继承自Thread类的run()方法
    public void run() {
        ...
        try {
            ...
            String as = (String)knownRequests.get("Counter");
            int aslen = as.length();
            ...
            //发会服务成功的状态
            errorResponse(oStream, "HTTP/1.1 200 OK", null, true, info);
            //生成首部域Content-Length
...
//                oStream.write(CRLF.getBytes());
                oStream.write((CRLF + "<h5>"+ as + "</h5>")	.getBytes());
                oStream.write((CRLF + "<h5>"+ aslen + "</h5>")	.getBytes());
                oStream.write((CRLF + "<i>"+ responseStr + "</i>")	.getBytes());
//                oStream.write((CRLF	+ "<html><head><title> HTTP/1.1 200 OK </title></head><body>"
//                        + "<i>"+ responseStr + "</i>"
//                        + "</body></html>")	.getBytes());
